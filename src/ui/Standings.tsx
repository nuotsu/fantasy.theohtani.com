...
export default async function Standings({ league }: { league: YF.LeagueInfo }) {
	...
	return (
		<>
			<header className="gap-y-ch p-ch gap-x-lh glass @container sticky top-0 z-1 flex flex-wrap items-center justify-between text-center">
				...
				<div className="gap-ch ml-auto flex items-center">
					<Revalidate
						revalidate={async () => {
							'use server'
							revalidateTag('yf')
					...
					<a className="action" href={league.url}>
						<FaYahoo />
						<span className="@max-lg:hidden">Open in Yahoo</span>
					</a>
				</div>
			</header>
